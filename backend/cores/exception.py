...
class EntityNotAllowed(FastShipError):
    """Some data is not allowed"""
...
def _get_handler(status: int, detail: str):
    ...
    def handler(request: Request, exception: Exception) -> Response:
        ...
        request.state.handled_exception = {
...
def add_exception_handlers(app: FastAPI):
    ...
    def response_validation_error_handler(request, exception) :
        error_detail = exception.errors()[0].get('ctx') if exception.errors() and exception.errors()[0].get('ctx') else str(exception.errors())
        ...
        return JSONResponse(
        content={"detail": f"""there are invalided data, {exception.errors()[0].get('ctx') if exception.errors() and exception.errors()[0].get('ctx') else ''}"""},
