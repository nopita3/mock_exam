<template>
  <AuthLayout>
    <div class="max-w-md w-full bg-white rounded-xl shadow-lg p-8">
      <button @click="$router.push('/login/teacher')" class="text-sm text-gray-500 hover:text-gray-700 mb-4 inline-flex items-center gap-1">
        <ArrowLeft class="w-4 h-4" /> Back to Teacher Login
      </button>
      <h2 class="text-2xl font-bold mb-2">Teacher Registration</h2>
      <p class="text-sm text-gray-500 mb-6">Use only the registration link shared by an admin.</p>

      <form @submit.prevent="handleRegister" class="space-y-4">
        <input v-model="form.userID" type="text" placeholder="Teacher ID" required class="input-field w-full" />
